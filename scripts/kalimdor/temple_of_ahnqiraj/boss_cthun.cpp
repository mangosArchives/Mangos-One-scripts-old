...
// ToDo: move the following scripts to eventAI
enum TentacleSpells
{
    // Tentacle spells which can be moved to eventAI
    // All Tentacles Spells
    ...
    //SPELL_HAMSTRING               = 26141,                // already defined

    // Giant Eye Tentacles
    //SPELL_EYE_BEAM                = 26134,                // already defined
};

struct MANGOS_DLL_DECL eye_tentacleAI : public Scripted_NoMovementAI
{
    eye_tentacleAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature) { Reset(); }
...
{
    claw_tentacleAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature) { Reset(); }
...
{
    giant_eye_tentacleAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature) { Reset(); }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiBeamTimer < uiDiff)
        {
            ...
            if (target)
                DoCastSpellIfCan(target, SPELL_EYE_BEAM);
...
{
    flesh_tentacleAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature) { Reset(); }
